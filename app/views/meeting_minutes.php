...
if ($mode === 'view') {
    ...
    [$attachmentErrors, $attachmentNotice, $attachmentToken] = handle_attachment_upload('documents', $record['id'], 'meeting_minutes_attachment_csrf', $canUploadAttachments);
    if (!empty($attachmentNotice) && empty($attachmentErrors)) {
        header('Location: ' . YOJAKA_BASE_URL . '/app.php?page=meeting_minutes&mode=view&id=' . urlencode($record['id']));
        exit;
    }
    ...
    if ($downloadRequested) {
        ...
        echo "<html><head><meta charset=\"UTF-8\"><title>Meeting Minutes</title><link rel=\"stylesheet\" href=\"" . YOJAKA_BASE_URL . "/assets/css/style.css\"></head><body>";
    ...
    ?>
    <div class="card">
        <h3>Meeting Minutes #<?= htmlspecialchars($record['id'] ?? ''); ?></h3>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $mode === 'create') {
    ...
    if (!$submittedToken || !hash_equals($_SESSION['meeting_minutes_csrf'], $submittedToken)) {
    ...
    if (empty($errors) && $selectedTemplate) {
        ...
        header('Location: ' . YOJAKA_BASE_URL . '/app.php?page=meeting_minutes&mode=view&id=' . urlencode($docId));
        exit;
    }
}
...
if ($mode === 'create') {
    ...
    <div class="form-grid">
        <div class="card">
            <h3>Select Meeting Minutes Template</h3>
            <form method="get" action="<?= YOJAKA_BASE_URL; ?>/app.php">
                <input type="hidden" name="page" value="meeting_minutes">
        ...
        <div class="card">
            ...
            <?php endif; ?>
            <form method="post" action="<?= YOJAKA_BASE_URL; ?>/app.php?page=meeting_minutes&amp;mode=create">
                ...
                <?php if (!empty($selectedTemplate['extra_sections'])): ?>
                    ...
                    <?php endforeach; ?>
                <?php endif; ?>
                <button type="submit">Generate Meeting Minutes</button>
            </form>
        </div>
        <?php endif; ?>
    </div>
...
<div class="actions" style="display:flex; justify-content: space-between; margin-bottom:10px; gap:10px; align-items:center;">
    <form method="get" action="<?= YOJAKA_BASE_URL; ?>/app.php" style="display:flex; gap:6px; align-items:center;">
        <input type="hidden" name="page" value="meeting_minutes">
        <input type="text" name="q" placeholder="Search meeting minutes" value="<?= htmlspecialchars($search); ?>">
        <button type="submit">Search</button>
    </form>
    <a class="button" href="<?= YOJAKA_BASE_URL; ?>/app.php?page=meeting_minutes&amp;mode=create">Create New Meeting Minutes</a>
</div>
<div class="table-responsive">
    <table class="table">
        <thead>
        ...
        <tbody>
            <?php if (empty($pagination['items'])): ?>
                <tr><td colspan="5">No meeting minutes found.</td></tr>
            <?php else: ?>
                <?php foreach ($pagination['items'] as $item): ?>
                    <tr>
                        ...
                        <td><a href="<?= YOJAKA_BASE_URL; ?>/app.php?page=meeting_minutes&amp;mode=view&amp;id=<?= urlencode($item['id'] ?? ''); ?>">View</a></td>
                    </tr>
                <?php endforeach; ?>
            <?php endif; ?>
        </tbody>
...
<div class="pagination">
    <?php for ($i = 1; $i <= ($pagination['total_pages'] ?? 1); $i++): ?>
        <a class="<?= ($i === ($pagination['page'] ?? 1)) ? 'active' : ''; ?>" href="<?= YOJAKA_BASE_URL; ?>/app.php?page=meeting_minutes&amp;p=<?= $i; ?>&amp;q=<?= urlencode($search); ?>">Page <?= $i; ?></a>
    <?php endfor; ?>
</div>
<?php endif; ?>
